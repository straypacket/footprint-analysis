...
# Create table
cur.execute("CREATE TABLE footprint_stats (day integer, mac_address varchar(18), timeslot_timings integer, avg_daily_power integer, avg_visit_duration integer, nreqs integer, nvisits integer, total_minutes integer, timeslots hstore);")
...
for day_key in days.keys():
	for mac_key in days[day_key].keys():
	  ...
	  cur.execute("INSERT INTO footprint_stats (day, mac_address, timeslot_timings, avg_daily_power, avg_visit_duration, nreqs, nvisits, total_minutes, timeslots) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
